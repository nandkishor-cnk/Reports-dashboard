...
function istToUtc(dateStr: Date | string | null): string | null {
    ...
    // If it's a JS Date object from Postgres driver, it's parsed as local time.
    // The DB data is in IST but Postgres driver might think it's local (UTC).
    // So let's format it and force it to explicitly be IST, then convert to UTC.
    let d;
    if (dateStr instanceof Date) {
        // luxon fromJSDate takes it as native. better to stringify without Z
...
// ── Teams ─────────────────────────────────────────────────────────────────────
const _NAME_FROM_EMAIL: Record<string, string> = {
...
async function runSync() {
    ...
    try {
      ...
      ORDER BY created_on
    `;
        const leadRes = await teleCrm.queryObject(leadsQuery, [leadsSinceIst]);
        const leadRows = leadRes.rows.map((row: any) => ({
        ...
        const allLeadIds = Array.from(new Set(leadRows.map((r: any) => r.telecrm_id)));
        if (allLeadIds.length === 0) {
            // In case no new leads came, fetch Q1 subset for joining calls and tasks just in case
            const allRes = await teleCrm.queryObject(`
      ...
      `, ["2026-01-01", "2026-03-31", ALL_TEAM_EMAILS]);
            allLeadIds.push(...allRes.rows.map((r: any) => r.id));
        }
        ...
        WHERE TRIM(leadid) = ANY($1)
          AND created_on >= $2
      `;
            const callsRes = await teleCrm.queryObject(callsQuery, [allLeadIds, callsSinceIst]);
            const callRows = callsRes.rows.map((row: any) => {
                let direction = null;
      ...
      `;
            const tasksRes = await teleCrm.queryObject(tasksQuery, [ALL_TEAM_EMAILS, tasksSinceIst]);

            const fastLeadIdSet = new Set(allLeadIds);

            const taskRows = tasksRes.rows.map((row: any) => {
                ...
                if (row.status_raw === "Done") {
                    ...
                    status_norm = "completed";
                    if (row.deadline) {
                        // Late = done after deadline; mark completed_at as deadline + 1s so SLA fails 
